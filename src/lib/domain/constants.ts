...
/**
 * 수도권 우선 배차 기준 위도 (R-18) — 이 선보다 남쪽은 **후순위**
 *
 * 현업 확인(2026-09-16): "천안 아래쪽이 아닌 위쪽으로, 수도권 쪽으로 배차될 수 있도록."
 * 36.90은 평택센터(36.99)와 천안(36.81) 사이에 그은 선으로, **천안시 자체가 남쪽에 들어간다**.
 * 실데이터에서 남쪽으로 걸리는 곳은 천안·아산·청주·익산 4곳뿐이다.
 *
 * 금지가 아니라 후순위다("되도록이면"). 북쪽 물량을 먼저 태우고,
 * 그래도 빈 회전이 남으면 남쪽을 싣는다 — 익산 1,200박스를 버리는 것이 목적이 아니다.
 */
export const METRO_SOUTH_LIMIT_LAT = 36.9;

/**
 * 남쪽 물량 벌점 가중치 (R-18)
 * 조합 점수의 1순위인 미적재 용량(0~1)과 같은 스케일에 둔다.
 * 0.6은 "적재율을 60%p 깎아먹는 것과 맞먹는 불이익" — 북쪽 대안이 있으면 반드시 북쪽을 고르고,
 * 대안이 없으면 남쪽이라도 싣는 세기다.
 */
export const SOUTH_PENALTY_WEIGHT = 0.6;
